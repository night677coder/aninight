          ...
          >
            <div className={`relative w-full flex-1 rounded-lg overflow-hidden bg-[#18181b] aspect-video ${isWatched ? 'opacity-60' : ''}`}>
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="hidden group-hover:flex items-center justify-center opacity-0 bg-white hover:bg-gray-200 rounded-full shadow group-hover:opacity-90 w-12 h-12 transition-all">
                  <svg xmlns="http://www.w3.org/2000/svg" className='play-buttonicon w-5 h-5 text-black' viewBox="0 0 24 24"><path fill="currentColor" d="M21.409 9.353a2.998 2.998 0 0 1 0 5.294L8.597 21.614C6.534 22.737 4 21.277 4 18.968V5.033c0-2.31 2.534-3.769 4.597-2.648z" /></svg>
                </div>
              </div>
            </div>
            <span className="absolute bottom-2 left-2 bg-black bg-opacity-80 px-[6px] py-[3px] text-xs rounded-md font-medium">{"EP " + episode?.number}</span>
            {episode?.isFiller===true && 
            <span className="absolute bottom-2 right-2 bg-white text-black px-[6px] py-[3px] text-xs rounded-md font-bold">F</span>
            }
            <span
