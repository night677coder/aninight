...
function MediaCard({ anime, session, mediaType = 'anime' }) {
    ...
    return (
        <div className='flex flex-col'>
            <div className="relative overflow-hidden rounded-lg shadow-lg group p-1 flex-shrink-0 group cursor-pointer">
                <Link href={isManga ? `/manga/info/${anime?.mediaId}` : `/anime/info/${anime?.mediaId}`}>
                    ...
                    <span className='absolute bottom-2 group-hover:flex hidden justify-center w-full text-center text-[12px]'>
                        {isManga ? `CH - ${anime?.progress}/${anime?.media?.chapters || '?'}` : `EP - ${anime?.progress}/${anime?.media?.episodes || '?'}`}
                    </span>
                </Link>
