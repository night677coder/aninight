...
function PlayerEpisodeList({ id, data, onprovider, setwatchepdata, epnum }) {
  ...
  useEffect(() => {
    const fetchepisodes = async () => {
      try {
        // Use the selected provider (onprovider) or default to animepahe
        const provider = onprovider || 'animepahe';
        const response = await getEpisodes(id, null, data?.status, false, 0, provider);
