...
export function ChromeCast({ tooltipPlacement, offset }: MediaButtonProps) {
  const handleCastClick = (e: React.MouseEvent | React.TouchEvent) => {
    e.preventDefault();
    e.stopPropagation();
    // Trigger custom cast event that will be handled in player.js
    const event = new CustomEvent('customCastRequest');
