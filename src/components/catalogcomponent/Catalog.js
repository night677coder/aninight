...
function Catalog({ searchParams }) {
    const { year, season, format, genre, search } = searchParams;
    ...
    }, [year, season, format, genre, search]); // removed sortby
    ...
    const isFormEmpty = !selectedYear && !seasonvalue && !formatvalue && genrevalue.length === 0 && !query && !searchvalue;
